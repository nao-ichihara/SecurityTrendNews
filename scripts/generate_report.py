...
収集した記事それぞれに1〜10の重要度スコアを付けてください（Grok話題性分析がある場合はそのトピックが自己申告しているスコアを優先的に採用し、それ以外の候補は報道量・影響規模・技術的新規性・規制/政策への波及度を基準に自分で採点する）。
固定のカテゴリ件数枠は使わず、次の手順でTop10を選定してください:
1. 5カテゴリ（Cyber Security / AI Risk / Data & Privacy / Security Governance / Crypto Currency）それぞれから最もスコアの高い候補を1件ずつ選び、5枠を確保する
2. 残り5枠は、①で選んだ5件を除く全候補をスコア降順で並べ、カテゴリを問わず上位から埋める
3. 1カテゴリの上限は7件までとする
この結果、カテゴリごとの件数はその日の話題性に応じて変動してよい（例: AI Riskが5件、Data & Privacyが1件、など）。ただしCrypto Currencyだけは必ず最低1件は含めること。
...
（スコアベースで選定した件数だけ、同形式で繰り返す。件数はカテゴリごとに固定しない）
